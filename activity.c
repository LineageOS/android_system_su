...
int send_intent(struct su_initiator *from, struct su_request *to, const char *socket_path, int allow, int type)
{
    ...
    sprintf(command, "am broadcast -a %s --es socket %s --ei caller_uid %d --es caller_bin %s --ei desired_uid %d --ei allow %d --ei version_code %d > /dev/null", action, socket_path, from->uid, from->bin, to->uid, allow, VERSION_CODE);
